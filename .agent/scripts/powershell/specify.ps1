...
$SHORT_NAME = ""
$BRANCH_NUMBER = ""
$REMAINING_ARGS = New-Object System.Collections.Generic.List[string]
...
for ($i = 0; $i -lt $args.Count; $i++) {
  ...
  switch ($arg) {
    ...
    default { $REMAINING_ARGS.Add($arg); continue }
  }
}
...
$FEATURE_DESCRIPTION = ($REMAINING_ARGS -join " ").Trim()
...
function Find-RepoRoot {
  ...
  while ($true) {
    if (Test-Path -LiteralPath (Join-Path $dir "specs") -PathType Container) { return $dir }
...
# ---------------------------
# Prefer the current working directory ($PWD) if it contains a specs/ dir
# or looks like a project root. This avoids resolving to a parent git repo.
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$HAS_GIT = $false
$repoRoot = $null

# 1. Check $PWD first (where the user invoked the command from)
$cwd = (Get-Location).Path
if (Test-Path -LiteralPath (Join-Path $cwd "specs") -PathType Container) {
  $repoRoot = $cwd
}
elseif (Test-Path -LiteralPath (Join-Path $cwd ".agent") -PathType Container) {
  $repoRoot = $cwd
}

# 2. Fall back to walking up from script directory
if ([string]::IsNullOrWhiteSpace($repoRoot)) {
  $repoRoot = Find-RepoRoot -StartDir $scriptDir
}

# 3. Last resort: git repo root
if ([string]::IsNullOrWhiteSpace($repoRoot)) {
  $repoRoot = Get-GitRepoRoot
}

if ([string]::IsNullOrWhiteSpace($repoRoot)) {
  [Console]::Error.WriteLine("Error: Could not determine project root. Please run this script from within the project directory.")
  exit 1
}

if (-not (Test-Path -LiteralPath (Join-Path $repoRoot ".git") -PathType Container)) {
  [Console]::Error.WriteLine("[specify] Warning: Project folder ($repoRoot) does not contain a local .git index.")
  [Console]::Error.WriteLine("[specify] You are inheriting branches from a parent directory's Git repository.")
  [Console]::Error.WriteLine("[specify] Please initialize local Git ('git init') to isolate projects.")
}

# Check if git is available (for branch creation)
try { $null = (git rev-parse --show-toplevel 2>$null); $HAS_GIT = ($LASTEXITCODE -eq 0) } catch { $HAS_GIT = $false }
...
$template = Join-Path (Join-Path $repoRoot ".agent\skills\speckit.specify") (Join-Path "templates" "spec-template.md")
